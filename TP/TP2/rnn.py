...
def train(mod):
    ...
    for epoch in range(nepochs):
        ...
        for data in trainloader:
            ...
            loss = crit(haty,goldy)
            totloss += loss.item()
            trainlossRNN.append(totloss)
            nbatch += 1
            loss.backward()
            optim.step()
        totloss /= float(nbatch)
        #print("epoch",epoch,"trainloss",totloss,"testloss",testloss)
...
#loss plot
plt.plot(trainlossRNN, label='train loss')
plt.plot(testlossRNN, label='test loss')
plt.legend()
plt.show()
